...
describe('GET /todos/:id', () => {
  ...
  it('should return a 404 if todo not found', done => {
    ...
    request(app).get(`/todos/${id.toHexString()}`).set('x-auth', users[0].tokens[0].token).expect(404).end(done);
  });

  it('should return a 404 for non-object ids', done => {
    request(app).get('/todos/000000').set('x-auth', users[0].tokens[0].token).expect(404).end(done);
  });
});
...
describe('DELETE /todos/:id', () => {
  ...
  it('should not delete another users todo', done => {
    ...
    request(app).delete(`/todos/${hexId}`).set('x-auth', users[1].tokens[0].token).expect(404).end(err => {
      if (err) return done(err);
  ...
  it('should return a 404 for non-object ids', done => {
    request(app).delete('/todos/000000').set('x-auth', users[1].tokens[0].token).expect(404).end(done);
  });
});
...
describe('PATCH /todos/:id', () => {
  ...
  it('should return a 404 if todo not found', done => {
    // make sure you get a 404 back
    const id = new ObjectID();
    request(app).patch(`/todos/${id.toHexString()}`).set('x-auth', users[0].tokens[0].token).expect(404).end(done);
  });

  it('should return a 404 for non-object ids', done => {
    request(app).patch('/todos/000000').set('x-auth', users[0].tokens[0].token).expect(404).end(done);
  });
});
...
describe('DELETE /users/me/token', () => {
  it('should remove auth token on logout', done => {
    request(app).delete('/users/me/token').set('x-auth', users[0].tokens[0].token).expect(200).end((err, result) => {
      if (err) {
        return done(err);
      }
